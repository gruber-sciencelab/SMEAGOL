from smeagol.variant import variant_effect_on_sites
...
records = [
    SeqRecord(seq=Seq("GACAAA"), id="seq0", name="seq0"),
...
def test_variant_effect_on_sites():
    ...
    assert _equals(
        result.max_score.values, np.array([3.33376361, 2.50816981, 2.50816981])
    )
    assert _equals(result.score.values, np.array([1.0, 0.769149, 0.769149]))
    assert _equals(
        result.variant_score.values, np.array([0.4080668, 0.1419828, 0.769149])
    )
