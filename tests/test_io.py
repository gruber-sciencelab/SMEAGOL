from smeagol.io import (read_fasta, write_fasta,
                        read_pms_from_file, read_pms_from_dir)
...
def test_read_fasta():
    ...
    assert [record.seq for record in records] == [
        record.seq for record in expected]
...
def test_read_pms_from_file():
    ...
    result = read_pms_from_file(
        input_file, matrix_type="PPM", check_lens=False, transpose=False,
        delimiter="\t"
    )
    expected = pd.DataFrame(
        {
            "Matrix_id": ["Matrix0", "Matrix1"],
            "probs": [
                np.array(
                    [
                        [0.97, 0.01, 0.01, 0.01],
...
def test_read_pms_from_dir():
    ...
    assert np.all(
        result.freqs[0]
        == np.array([[2, 18, 0, 0], [2, 16, 1, 1], [0, 2, 18, 0]],
                    dtype="float32")
    )
    assert np.all(
        result.freqs[1]
        == np.array(
            [[0, 0, 1, 19], [20, 0, 0, 0], [1, 4, 13, 2], [0, 1, 17, 2]],
            dtype="float32",
        )
    )
