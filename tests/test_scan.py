...
records = [
    SeqRecord(seq=Seq("ATTAAA"), id="S1.1", name="S1"),
...
def test_locate_sites():
    ...
    assert _equals(result.max_score.values,
                   np.array([3.33376361, 2.50816981]))
    assert _equals(result.frac_score.values,
                   np.array([0.70280927926, 0.76908668]))


def test_bin_sites_by_score():
    thresholded, scores = model.predict_with_threshold(encoding.seqs,
                                                       0.25, True)
    result = _bin_sites_by_score(
        encoding, model, thresholded, scores, [0.25, 0.5, 0.75]
    )
    fields = ["id", "name", "sense", "Matrix_id", "width"]
    expected = [
        ["S1.1", "S1.1", "S1.2", "S1.2", "S1.2", "S1.2"],
        ["S1"] * 6,
        ["+"] * 6,
        ["x", "z", "x", "z", "x", "z"],
        [3] * 6,
    ...
    for field, exp in zip(fields, expected):
        assert np.all(result[field].values == exp)
    assert np.all(
        [str(x) for x in result.bin]
        == ["(0.25, 0.5]"] * 2 + ["(0.5, 0.75]"] * 2 + ["(0.75, 1.0]"] * 2
    )
