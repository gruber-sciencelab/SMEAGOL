...
def test_PWMModel():
    ...
    assert _equals(model.max_scores, np.array([3.33376361,
                                               5.52770673, 2.50816981]))
    ...
    # Check weights
    assert _equals(
        np.array(model.model.layers[2].weights[0]),
        np.array(list(one_hot_dict.values())),
    )
    assert np.all(model.model.layers[4].weights[0].shape == (5, 4, 3))
    assert _equals(
        np.array(model.model.layers[4].weights[0][:, :, 0]),
        np.pad(x, ((0, 2), (0, 0)))
    )
    assert _equals(np.array(model.model.layers[4].weights[0][:, :, 1]), y)
    assert _equals(
        np.array(model.model.layers[4].weights[0][:, :, 2]),
        np.pad(z, ((0, 2), (0, 0)))
    )
    ...
    assert _equals(
        preds[0, 0, :], np.array([-1.60847875, -7.32647115,
                                  0.35611725000000005])
    )
