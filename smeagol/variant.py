...
def variant_effect_on_site(site, variants):
    ...
    Returns:
        site_variants (pd.DataFrame): variants that intersect with the site and their predicted effect.
...
def variant_effect_on_sites(sites, variants, seqs, pwms):
    ...
    # Get PWM
    all_sites['weights'] = [pwms.weights[pwms.Matrix_id==x].values[0] for x in all_sites.Matrix_id]
    
    # Get max scores
    all_sites['max_score'] = [np.sum(np.max(x, axis=1)) for x in all_sites.weights]
    
    # Score sites
    all_sites['ref_scores'] = all_sites.apply(lambda x: score_site(x.weights, x.seq, position_wise=True), axis=1)
