...
def _enrich_over_shuffled(real_counts, shuf_stats, background='binomial', records=None):
    """Function to test for enrichment of binding sites in real vs. shuffled genomes
...
def enrich_in_genome(records, model, rcomp, threshold, simN=1000, simK=2, sense='+', background='binomial', verbose=False, combine_seqs=True, seq_batch=0, shuf_seqs=None):
    """Function to calculate enrichment of motifs in sequence(s) relative to a background. The background sequences can be supplied directly or generated by shuffling the given sequence(s).
        
    Args:
        records (list): A list of seqrecord objects containing the sequence(s) to test.
        ...
        threshold (float): Threshold motif match score to use for PWM scanning. A sequence that has a PWM match score higher than this will be counted as a match.
        simN (int): The number of shuffled sequences to generate
        ...
        seq_batch (int): The number of shuffled sequences to scan at a time. If 0, scan all.
        shuf_seqs (str/list): A path to FASTA file or list of seqrecord objects containing shuffled sequences
    ...
    enr = _enrich_over_shuffled(real_preds['counts'], shuf_preds['stats'], background=background, records=records)
...
def examine_thresholds(records, model, simN=300, simK=2, rcomp='only', min_threshold=0.5, threshold_shift=0.1, sense='+', verbose=False, combine_seqs=True):
    ...
    Args:
        ...
        model (PWMModel): An object of class PWMModel for PWM scanning
        simN (int): The number of shuffled sequences to generate. Default is 300.
        simK (int): The k-mer frequency to conserve while shuffling. Default is 2.
        rcomp (str): 'only' to encode the sequence reverse complements, 'both' to encode the reverse
                     complements as well as original sequences, or 'none'. Default is 'only'.
...
def enrich_in_window(window, sites, genome, matrix_id=None, alternative='two-sided'):
    ...
    Returns:
        result (pd.DataFrame): Pandas dataframe containing the result of enrichment analysis in selected window.
    ...
    # Fisher's exact test
    odds, p = stats.fisher_exact([[result['count'], count_neg], [result.tot_count, tot_neg]], alternative=alternative)
    result['odds'] = odds
    result['p'] = p
    return result


def enrich_in_sliding_windows(sites, genome, width, shift=None, matrix_id=None):
    ...
    Args:
        ...
        genome (list): A list of SeqRecord objects containing the sequence(s).
