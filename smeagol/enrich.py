...
def enrich_in_genome(records, model, rcomp, threshold, simN=1000, simK=2, sense='+', background='binomial', verbose=False, combine_seqs=True, seq_batch=0, shuf_seqs=None):
    ...
    Args:
        ...
        rcomp (str): 'only' to encode the reverse complements of the given sequences, 'both' to encode the reverse
                     complements as well as the given sequences, or 'none' to use only the given sequences without reverse complementing.
...
def examine_thresholds(records, model, simN=300, simK=2, rcomp='only', min_threshold=0.5, threshold_shift=0.1, sense='+', verbose=False, combine_seqs=True):
    ...
    Returns:
        results (dict): dictionary containing results. Contains the following keys: 'real_binned' (Number of motif matches for each PWM in the given sequences, binned by their match score), 'shuf_binned' (Number of motif matches for each PWM in the background sequences, binned by their match score)
    """
...
def enrich_in_sliding_windows(sites, genome, width, shift=None, matrix_id=None, alternative='two-sided'):
    ...
    windows = windows.apply(enrich_in_window, axis=1, args=(sites, genome, matrix_id, alternative)).reset_index(drop=True)
